 		...
 		for (var i = 0; i < elementsLength; i++) {
			...
			if(fundDivs(elements[i])&&(parseInt(elements[i].style.zIndex)>=parseInt(caller.style.zIndex))
				...
				if(leftOff==null||bottOff==null||height==null||width==null||caller.MyWidth==null||
		 		...
		 		if(((elements[i].className).toString()=="splitDIVofoghi"||(elements[i].className).toString()=="splitDIVamoodi")){
...
//				alert("newOn "+newOn.style.width+" "+newOn.style.height+" "+newOn.style.left+" "+newOn.style.bottom);
		...
		newDiv.style.position="relative";
 		newDiv.style.width=caller.style.width;
 		newDiv.style.height=caller.style.height;
 		if(((caller.className).toString()=="splitDIVofoghi"||(caller.className).toString()=="splitDIVamoodi")){
 			newDiv.style.height=(PxToInt(newDiv.style.height)+innerSplitOffsetHeight).toString()+"px";
 			newDiv.style.width=(PxToInt(newDiv.style.width)+innerSplitOffsetWidth).toString()+"px";
		}
 		newDiv.style.width=(PxToInt(newDiv.style.width)/newWidthScale)+"px";
 		newDiv.style.height=(PxToInt(newDiv.style.height)/newHeightScale)+"px";
...
//			textfillLeft(innerDiv1);
	...
	function write4sides(check,Zindex,divCreate,startSrc,endSrc,startDst,endDst){
		...
		if(check&&PxToInt(divCreate.style.width)>maxLimit&&PxToInt(divCreate.style.height)>maxLimit){
			...
			var innerDiv3=document.createElement("span");
			var innerDiv4=document.createElement("span");
			innerDiv1.textContent=endSrc;
			innerDiv2.textContent=startSrc;
			innerDiv3.textContent=startDst;
			innerDiv4.textContent=endDst;
			outerDiv.appendChild(innerDiv1);
			outerDiv.appendChild(innerDiv2);
			outerDiv.appendChild(innerDiv3);
			outerDiv.appendChild(innerDiv4);
			...
			innerDiv1.style.fontSize="20px";
			innerDiv2.style.fontSize="20px";
			innerDiv3.style.fontSize="20px";
			innerDiv4.style.fontSize="20px";
...
//						alert(srcLength+"  "+dstLength);
	...
	function textfillLeft (obj) {
	        ...
	        var maxHeight = ((parent).style.height);
	        var maxWidth = ((parent).style.width);
	        var textHeight;
	        var textWidth;
	        do {
	        	obj.style.fontSize =fontSize.toString()+"px";
	        	textHeight = (obj.style.height);
	            textWidth =  (obj.style.width);
	            fontSize = fontSize - 1;
	        } while ((textWidth >maxHeight|| textHeight >maxWidth/5)&& fontSize > 1);
	        fontSize = fontSize + 1;
	...
	function textfillDown (obj) {
	        ...
	        do {
	        	...
	        	textHeight = (parent.style.height);
	            textWidth =  (parent.style.width);
	            fontSize = fontSize - 1;
	            alert("pause "+textWidth+" "+maxWidth+"\n"+textHeight+" "+maxHeight);
	        } while ((textWidth >maxWidth/5|| textHeight >maxHeight/5)&& fontSize > 1);
	        fontSize = fontSize + 1;
...
//		var elements = document.getElementsByClassName('innerDivLeft');
//		var elementsLength = elements.length;
//		for (var i = 0; i < elementsLength; i++) {
//		 	textfillLeft(elements[i]);
//		}
//		elements = document.getElementsByClassName('innerDivDown');
//		elementsLength = elements.length;
//		for (i = 0; i < elementsLength; i++) {
//			textfillDown(elements[i]);
//		}
